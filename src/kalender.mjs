...
/* Anlässe, die ohne festen Prüfungstermin gelten: der Rhythmus des Jahres.
   Für Jura sind das die Klausurenkampagnen im Frühjahr und Herbst, der Beginn
   des Referendariats und der Semesterwechsel. */
function jahresAnlaesse(jahr) {
  const d = (m, t) => `${jahr}-${String(m).padStart(2, "0")}-${String(t).padStart(2, "0")}`;
  return [
    { datum: d(1, 8), art: "auftakt", titel: "Neues Jahr, neuer Lernplan", kontext: "Zum Jahresbeginn stellen viele ihren Examensplan auf: Stoffverteilung, Klausurenkurs, Wiederholungsrhythmus." },
    { datum: d(3, 1), art: "kampagne", titel: "Frühjahrskampagne", kontext: "In vielen Ländern beginnt jetzt die Klausurenkampagne der ersten juristischen Prüfung. Ton: ruhig, Klausurtechnik, keine neuen Themen." },
    { datum: d(5, 1), art: "referendariat", titel: "Start ins Referendariat", kontext: "Zum Mai beginnt in vielen Ländern ein neuer Referendarjahrgang – Aktenvortrag, Relationstechnik und die Umstellung vom Gutachten- auf den Urteilsstil." },
    { datum: d(8, 15), art: "kampagne", titel: "Herbstkampagne", kontext: "Die zweite Klausurenkampagne des Jahres steht an. Jetzt zählen Dauerbrenner, Klausuren unter Zeit und Nerven." },
    { datum: d(10, 1), art: "semesterstart", titel: "Semesterstart", kontext: "Das Wintersemester beginnt; Erstsemester steigen ein, Examenskandidaten gehen in die Vorbereitung." },
    { datum: d(12, 31), art: "jahreswechsel", titel: "Jahreswechsel", kontext: "Zum Jahreswechsel treten Gesetzesänderungen in Kraft; für das Examen zählt der Rechtsstand, den die Prüfung vorgibt." },
  ];
}
...
export function anlaesse(datum = new Date().toISOString().slice(0, 10)) {
  ...
  /* Ohne festen Prüfungstermin (Jura: je Land und Durchgang verschieden) gibt
     es keine Countdown- und Prüfungstags-Anlässe – nur den Jahresrhythmus. */
  if (!CONFIG.examen.schriftlich) return jahresAnlaesse(jahr);
  ...
  liste.push(
    { datum: pruefung.tag1, art: "pruefungstag", titel: "Klausurtag Zivilrecht", kontext: "Heute ist ein Klausurtag im Zivilrecht. Ton: ruhig, ermutigend, ein letzter Hinweis zur Technik (Anspruchsgrundlage zuerst, Gutachtenstil durchhalten) – keine neuen Inhalte." },
    { datum: pruefung.tag2, art: "pruefungstag", titel: "Klausurtag Strafrecht", kontext: "Heute ist ein Klausurtag im Strafrecht. Ton: ermutigend, ein Hinweis zur Technik (Tatkomplexe trennen, Prüfungsreihenfolge einhalten) und zur Zeiteinteilung." },
...
export const MINDSET_THEMEN = [
  { id: "mindset-1", typ: "mindset", fach: "methodik", klausur: 1, prioritaet: "hoch", titel: "Prüfungsangst: Was am Abend vor der ersten Klausur wirklich hilft", normen: [], kern: { einordnung: ["Angst ist normal und sogar nützlich – solange sie nicht das Denken blockiert."], lernziele: ["ein konkretes Abendritual", "Schlaf vor Wiederholung", "Materialien am Vorabend packen"], merksatz: "Nichts Neues mehr lernen, nur noch Ruhe organisieren." } },
  { id: "mindset-2", typ: "mindset", fach: "methodik", klausur: 2, prioritaet: "hoch", titel: "Blackout in der Klausur: das 3-Schritte-Protokoll", normen: [], kern: { einordnung: ["Ein Blackout dauert Minuten, wenn man ein Protokoll hat – und die ganze Klausur, wenn nicht."], lernziele: ["Aufgabe wechseln statt starren", "Sachverhalt laut im Kopf gliedern", "mit dem Obersatz beginnen, nicht mit dem Ergebnis"], merksatz: "Wechseln, gliedern, Obersatz – dann kommt der Rest." } },
  { id: "mindset-3", typ: "mindset", fach: "methodik", klausur: 3, prioritaet: "hoch", titel: "Zeitdruck: Warum Punkte am Ende verloren gehen, nicht am Anfang", normen: [], kern: { einordnung: ["Fast alle verlieren Punkte, weil sie die Zulässigkeit zu schön schreiben."], lernziele: ["Minutenplan je Prüfungspunkt", "Unstreitiges knapp abhandeln", "Abbruchkriterium am Rand notieren"], merksatz: "Die Uhr entscheidet über die Note, nicht das Wissen." } },
  { id: "mindset-4", typ: "mindset", fach: "methodik", klausur: 1, prioritaet: "hoch", titel: "Perfektionismus: Der Bearbeitungsstil, der durchfallen lässt", normen: [], kern: { einordnung: ["Perfekte Teillösungen bringen weniger als vollständige, knappe Lösungen."], lernziele: ["jede Fallfrage anfassen", "Ergebnis zu jedem Anspruch, auch knapp", "erst Punkte sammeln, dann ausformulieren"], merksatz: "Vollständig schlägt perfekt." } },
  { id: "mindset-5", typ: "mindset", fach: "methodik", klausur: 2, prioritaet: "hoch", titel: "Lerngruppe: Warum du nicht allein lernen solltest", normen: [], kern: { einordnung: ["Wer erklärt, versteht. Wer sich vergleicht, erkennt Lücken."], lernziele: ["wöchentlicher Klausurtausch", "gegenseitiges Korrigieren mit Bewertungsbogen", "ein fester Termin"], merksatz: "Erklären ist die härteste Prüfung – und die beste Vorbereitung." } },
  { id: "mindset-6", typ: "mindset", fach: "methodik", klausur: 3, prioritaet: "hoch", titel: "Der Klausurtag: Ablauf von der Anreise bis zur Abgabe", normen: [], kern: { einordnung: ["Ein fester Ablauf nimmt Entscheidungen ab, die am Klausurtag Energie kosten."], lernziele: ["Frühstück, Anreise, Hilfsmittel prüfen", "erste 20 Minuten: Sachverhalt lesen und skizzieren, nicht schreiben", "Pausen bewusst nehmen"], merksatz: "Routine ist am Klausurtag mehr wert als jede Wiederholung." } },
  { id: "mindset-7", typ: "mindset", fach: "methodik", klausur: 1, prioritaet: "hoch", titel: "Nach einer schlechten Klausur: Wie du die nächste trotzdem rettest", normen: [], kern: { einordnung: ["Eine Klausur entscheidet nicht – der Durchschnitt zählt, und die nächsten sind noch offen."], lernziele: ["nicht nachlesen, nicht diskutieren", "Abend ohne Fachgespräch", "Schemata für den nächsten Tag durchgehen"], merksatz: "Was geschrieben ist, ist geschrieben. Die nächste Klausur ist ein neuer Anfang." } },
  { id: "mindset-8", typ: "mindset", fach: "methodik", klausur: 2, prioritaet: "hoch", titel: "Wie du mit Nichtwissen in der Klausur umgehst", normen: [], kern: { einordnung: ["Niemand weiß alles – Punkte gibt es für sauberen Aufbau auch bei unsicherem Ergebnis."], lernziele: ["Obersatz, Definition, Subsumtion, Ergebnis trotzdem", "mit vertretbarer Ansicht weiterprüfen und das sagen", "keine leeren Fallfragen"], merksatz: "Ein sauberer Weg zum vertretbaren Ergebnis bringt mehr als kein Weg." } },
];
...
/* Ohne festen Termin richtet sich die Phase nach dem Kalenderjahr: Kampagnen
   im Frühjahr und Herbst, dazwischen Vorbereitung. */
function jahresPhase(datum) {
  const monat = Number(datum.slice(5, 7));
  if (monat === 3 || monat === 4) return "Frühjahrskampagne – viele schreiben gerade ihre Examensklausuren: Klausurtechnik, Dauerbrenner, Nerven; nichts Neues aufmachen";
  if (monat === 8 || monat === 9) return "Herbstkampagne – zweite Klausurenrunde des Jahres: Wiederholung, Zeitmanagement, ruhiger Ton";
  if (monat === 5 || monat === 10) return "Neustart – neuer Referendarjahrgang bzw. Semesterbeginn: Grundlagen, Aufbauwissen, Lernorganisation";
  if (monat === 1 || monat === 2) return "Vorbereitungsphase – Lernplan, Stoffverteilung, Klausurenkurs; jetzt ist Zeit für Tiefe";
  return "reguläre Vorbereitungszeit – Examenswissen systematisch, Streitstände und Schemata";
}
...
export function phase(datum) {
  if (!CONFIG.examen.schriftlich) return jahresPhase(datum);
