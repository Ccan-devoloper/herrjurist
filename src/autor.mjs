...
import { CHARAKTER_NAMEN_LEGENDE, COVER_FALLNAMEN_REGEL } from "./charaktere.mjs";
...
- Fälle, Beispiele, Zahlen sowie Firmen-, Orts- und sonstige freie Namen erfindest du selbst – und jedes Mal neu: andere Branche, anderer Ort, andere freie Namen als in früheren Texten. Nie einen Namen aus dem Ausgangsmaterial oder der Sperrliste übernehmen. Die wiederkehrenden Cover-Charaktere sind die ausdrückliche Ausnahme von der Regel gegen wiederholte Personennamen.\n- ${COVER_FALLNAMEN_REGEL}
...
- Landesrecht kennzeichnen und bundesweit denken: Polizei- und Ordnungsrecht, Kommunalrecht, Bauordnungsrecht und Verwaltungsvollstreckung sind je Bundesland anders geregelt. Dargestellt werden die Grundsätze, die überall tragen; kein Landesparagraf wird als bundesweit gültig ausgegeben, und ein Beitrag handelt nie von der Rechtslage nur eines Bundeslandes. Wo eine konkrete Norm nötig ist, zeige den Ländervergleich: benenne das Rechtsinstitut abstrakt (die Generalklausel, die Kommunalverfassungsbeschwerde) und nenne beispielhaft zwei bis drei Landesnormen, wenn die Belegstelle sie liefert – sonst genügt „Nummer je nach Landesgesetz verschieden“. Dazu der Hinweis auf Folie oder in der Caption: „je nach Land unterschiedlich – prüf dein Landesrecht“.
...
function coverTextKurz(text) {
  ...
  const kurz = sauber.split(" ").slice(0, 6).join(" ");
  if (kurz.length <= 36) return kurz;
  const woerter = kurz.split(" ");
  while (woerter.length > 1 && woerter.join(" ").length > 36) woerter.pop();
