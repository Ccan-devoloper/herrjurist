...
/* Wie auf den Kacheln gilt die Schreibweise der Normen hier im Renderer,
   nicht in der Quelle: Auf einer Reel-Folie stand schon "Paragraf 48 VwVfG",
   weil die Regel fuer den Sprechertext auf den Bildschirmtext abgefaerbt
   hatte. Der Sprechertext selbst laeuft nicht hierdurch - er wird gesprochen,
   nicht gesetzt. */
...
/* Ein Punkt beendet nicht jeden Satz: „§ 48 Abs. 4 VwVfG“ zerfiel sonst nach
   „Abs.“ in zwei Untertitel. Einzelne Buchstaben („S.“, „f.“) und die üblichen
   juristischen Kürzel gelten deshalb nicht als Satzende. */
const ABKUERZUNGEN = new Set(["abs", "nr", "hs", "lit", "art", "ff", "f", "vgl", "bzw", "ca", "ggf", "inkl", "insb", "rn", "rspr", "sog", "usw", "etc", "evtl", "str", "hm", "aa", "mio", "mrd", "ders", "aao"]);
...
export function untertitelBloecke(szenen) {
  ...
  /* Gezaehlt wird in gesprochenen Woertern, angezeigt wird die Schreibfassung:
     Die Stimme sagt "Paragraf 48 Absatz 4 Verwaltungsverfahrensgesetz", auf
     der Karte steht "§ 48 Abs. 4 VwVfG". Ohne diesen Schritt lief der
     ausgeschriebene Gesetzesname quer ueber den halben Bildschirm. */
...
.untertitel .block{max-width:100%;overflow-wrap:anywhere;font-family:var(--titel);font-size:84px;line-height:1.1;text-transform:uppercase;letter-spacing:.01em;font-weight:${stil.schrift.titelGewicht};transform-origin:50% 50%;will-change:transform}
